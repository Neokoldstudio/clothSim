...
void ParticleSystem::computeForces()
{
    ...
    // TODO Initialize and compute the gravity acting on each particle.
    //
    ...
    // TODO For each spring, add the force of the spring to each particle
    //      Recall that the force acting on particle with index0 is equal and opposite
    //      the force acting on index1.
    //
    ...
    for (int i = 0; i < numSprings; i++)
    {
        ...
        Eigen::Vector3f deltaNormalized = delta.normalized(); // normalized delta vec
        float length = delta.norm();
        float projectedVel = (part1->v - part0->v).dot(deltaNormalized); // for damping : project the velocities onto the delta vector

        Eigen::Vector3f f = (currentSpring->k * (length - currentSpring->r) + currentSpring->b * (projectedVel)) * delta.normalized();
...
// TODO Assemble the state vector q, which consists of particle positions and velocities
void ParticleSystem::getState(Eigen::VectorXf &q) const
{
    ...
    for (int i = 0; i < numParticles; i++)
    {
        q.segment(6 * i, 3) = m_particles[i]->x;     // get the postion of the particle and puts it in the vector q
        q.segment(6 * i + 3, 3) = m_particles[i]->v; // get the velocity of the particle and puts it in the vector q
    }
}
